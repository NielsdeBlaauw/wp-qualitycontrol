...
class Cloned extends Base implements iFieldType{
  ...
  protected function parse_sub_fields(){
    ...
    foreach($this->field->get_sub_fields() as $sub_field_data){
      $sub_field = new \NDB\QualityControl\FieldDefinitions\ACF($sub_field_data);
      $this->sub_fields[$sub_field->get_key()] = FieldFactory::create_field($sub_field, $this->context);
    }
  }

  public function custom_meta_insert(int $id){
    \NDB\QualityControl\Command::$warnings[$this->field->get_key().'_custom_meta_unsupported'] = sprintf('Custom field %s has unsupported type Cloned.', $this->field->get_key());
  }
